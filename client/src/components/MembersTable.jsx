...
export default function MembersTable() {
  ...
  const {loading: membersLoading, data: membersData, refetch} = useQuery(LIST_MEMBERS, { variables: { page: currentPage, sortBy, order: sortAscending ? "ascending" : "decending" } });

  useEffect(() => {
    refetch({ sortBy, order: sortAscending ? "ascending" : "decending", page: currentPage, search: searchQuery});
  }, [sortBy, sortAscending, currentPage]);

  useEffect(() => {
    if(membersData) {
      setEntries(membersData.getMembers.members);
      setNumPages(membersData.getMembers.numPages);
    }
  }, [membersData]);
